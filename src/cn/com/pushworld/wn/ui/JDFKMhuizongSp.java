...
public class JDFKMhuizongSp extends AbstractWorkPanel implements ActionListener {
	...
	public void initialize() {
		...
		list.addBatchBillListButton(new WLTButton[] { updateButton, verifyButton, vertiybatchButton,backbatchButton });
	...
	private void vertiybatchData() {
		try {
			...
			if (notIds != null && !"".equals(notIds.trim())) {
				...
				UIUtil.executeUpdateByDS(null, "update WN_JDFKMHZB_02 set ratifyReason='" + inputValue + "',ratifyPerson='"+ratifyPerson+"',ratifyTime='"+ratifyTime+"',state='已审核' where id in (" + ids + ")");
			    list.refreshData();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void backbatchData() {//批量退回
		try{
			...
			if (notIds != null && !"".equals(notIds.trim())) {
				...
				String inputValue = JOptionPane.showInputDialog("请输入理由:");
				UIUtil.executeUpdateByDS(null, "update WN_JDFKMHZB_02 set ratifyReason='" + inputValue + "',ratifyPerson='"+ratifyPerson+"',ratifyTime='"+ratifyTime+"',state='已退回' where id in (" + ids + ")");
				list.refreshData();
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	private void QuickQuery() {//查询出的数据只能是柜员已经提交，已退回或者已审核的;
		try {
			...
			String code=UIUtil.getStringValueByDS(null, "SELECT code FROM pub_corp_dept WHERE ID='"+dept+"'");
			String condition = "1=1 " + list.getQuickQueryPanel().getQuerySQLCondition();
			String state = "未提交";
			condition = condition + " and STATE !='" + state + "' and JG_ID='"+code+"'";
	...
	//			BillCardDialog dialog = new BillCardDialog(list, "编辑", cardpanel, WLTConstants.BILLDATAEDITSTATE_UPDATE);
