...
public class BMqualitativeScorePanel extends AbstractWorkPanel implements BillTreeSelectListener, BillListSelectListener, ActionListener {
	...
	private String PFUSERNAME = ClientEnvironment.getInstance().getLoginUserName();
	private String PFSUERCODE = ClientEnvironment.getInstance().getLoginUserCode();
	private String PFUSERDEPT = ClientEnvironment.getInstance().getLoginUserDeptId();
	private String PFUSERDEPTNAME = ClientEnvironment.getInstance().getLoginUserDeptName();
	...
	public void initialize() {
		...
		billListPanel_Dept_check.addBatchBillListButton(new WLTButton[] { btn_save, btn_end });
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btn_save) {//保存
			try {
				...
				UpdateSQLBuilder update = new UpdateSQLBuilder(billListPanel_Dept_check.getTempletVO().getTablename());
				...
				for (int i = 0; i <= bos.length - 1; i++) {
					System.out.println("当前考核项:" + bos[i].getStringValue("xiangmu"));
					...
					if (bos[i].getStringValue("KOUFEN") != null) {
						KOUOFEN = Double.parseDouble(bos[i].getStringValue("KOUFEN"));
						if (FENZHI < KOUOFEN) {
							sb.append("第" + (count) + "行数据项目为[" + bos[i].getStringValue("XIANGMU") + "]扣分项大于分值   \n");
						}
					} else {
						sb.append("第" + (count) + "行数据项目为[" + bos[i].getStringValue("XIANGMU") + "]扣分项为空   \n");
					}
				...
				if (sb.length() > 0) {
					...
					billListPanel_Dept_check.setRealValueAt(String.valueOf(100 - result),bos.length-1, "KOUFEN");
					update.setWhereCondition("xiangmu='" + bos[0].getStringValue("xiangmu") + "' and deptname='" + bos[0].getStringValue("deptname") + "'");
					update.putFieldValue("KOUFEN", 100.0 - result);
					list.add(update.getSQL());
					UpdateSQLBuilder update2 = new UpdateSQLBuilder(billListPanel_Dept_check.getTempletVO().getTablename());
					String checkName = bos[1].getStringValue("xiangmu");
					update2.setWhereCondition("xiangmu like'" + checkName.substring(0, checkName.indexOf("-")) + "%' and deptname='" + bos[1].getStringValue("deptname") + "'");
			...
			try {
				String khDeptName="";
				String[] state = UIUtil.getStringArrayFirstColByDS(null, "select state from wn_BMPF_table where state='评分中'");
				if (state.length <= 0) {
				...
				BillVO[] bos = billListPanel_Dept_check.getBillVOs();
				String sql = "select * from wn_BMPF_table where 1=1 and deptname='" + vo.getStringValue("name") + "' and state='评分中' ";
				String deptN = "";
				if ("客户服务部".equals(PFUSERDEPTNAME)) {
				...
				if (vos.length > 0) {//存在未评分的项
					if (MessageBox.confirm(this, "以下评分未完成,确定强制结束吗?")) {
						...
						for (int i = 0; i < bos.length; i++) {
							...
							UpdateSQLBuilder update = new UpdateSQLBuilder(billListPanel_Dept_check.getTempletVO().getTablename());
							update.setWhereCondition("1=1 and xiangmu ='"+bos[i].getStringValue("xiangmu")+"' AND DEPTNAME='" + bos[i].getStringValue("deptname") + "' and state='评分中'");
							...
							if (KOUFEN == null || "".equals(KOUFEN) || KOUFEN.isEmpty()) {
								KOUFEN = "0.0";
							}
							if (Double.parseDouble(KOUFEN) > Double.parseDouble(KOUFEN)) {
								KOUFEN = FENZHI;
							}
						...
						UpdateSQLBuilder update2 = new UpdateSQLBuilder(billListPanel_Dept_check.getTempletVO().getTablename());
						update2.setWhereCondition("1=1 and xiangmu ='" + deptN + "' and  DEPTNAME='" + vo.getStringValue("name") + "' and FENZHI IS NULL");
					...
					for (int i = 0; i < bos.length; i++) {
						...
						System.out.println(bos[i].getStringValue("xiangmu")+":"+KOUFEN);
			    ...
			    billListPanel_Dept_check.queryDataByCondition("1=1 and state='评分结束' and deptname='"+khDeptName+"' and xiangmu like '"+deptN+"%' and PFTIME='"+maxTime+"'","fenzhi");
			} catch (Exception e2) {
				e2.printStackTrace();
			}
		}
	...
	public void onBillTreeSelectChanged(BillTreeSelectionEvent e) {
		try {
			String deptName = ClientEnvironment.getInstance().getLoginUserDeptName();//当前登录人机构
			String selectedDeptName = e.getCurrSelectedVO().getStringValue("name");//获取到选中部门
			...
			if ("客户服务部".equals(deptName)) {
				...
				billListPanel_Dept_check.queryDataByCondition("1=1 and deptname='" + selectedDeptName + "' and xiangmu like '内控合规%' and PFTIME='"+maxTime+"'", "fenzhi");
				String state=UIUtil.getStringValueByDS(null, "select state from WN_BMPF_TABLE WHERE deptname='" + selectedDeptName + "' and xiangmu like '内控合规%'  and PFTIME='"+maxTime+"' AND  ROWNUM=1");
			    if("评分结束".equals(state)){
			    	billListPanel_Dept_check.setItemEditable("KOUFEN", false);
			    	billListPanel_Dept_check.setItemEditable("mention", false);
			    }else{
				...
				billListPanel_Dept_check.queryDataByCondition("1=1 and deptname='" + selectedDeptName + "' and xiangmu like '安全保卫%' and  PFTIME='"+maxTime+"'", "fenzhi");
				String state=UIUtil.getStringValueByDS(null, "select state from WN_BMPF_TABLE WHERE deptname='" + selectedDeptName + "' and xiangmu like '安全保卫%'  and PFTIME='"+maxTime+"' AND  ROWNUM=1");
			    if("评分结束".equals(state)){
			    	billListPanel_Dept_check.setItemEditable("KOUFEN", false);
			    	billListPanel_Dept_check.setItemEditable("mention", false);
			    }else{
				...
				billListPanel_Dept_check.queryDataByCondition("1=1 and deptname='" + selectedDeptName + "' and xiangmu like '党建%' and PFTIME='"+maxTime+"'", "fenzhi");
				String state=UIUtil.getStringValueByDS(null, "select state from WN_BMPF_TABLE WHERE deptname='" + selectedDeptName + "' and xiangmu like '党建%'  and PFTIME='"+maxTime+"' AND  ROWNUM=1");
			    if("评分结束".equals(state)){
			    	billListPanel_Dept_check.setItemEditable("KOUFEN", false);
			    	billListPanel_Dept_check.setItemEditable("mention", false);
			    }else{
