 ...
 * @author zzl
 *贷款户数新增完成比
 * 2019-5-24-下午03:28:07
 */
public class LoanNewlyIncreasedWKPanel extends AbstractWorkPanel implements ActionListener{
		...
		if(actionevent.getSource()==list.getQuickQueryPanel()){
			...
			try {
				...
				if(data.length<=0){
					...
					String str=service.getDKHouseholdsXZ(list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").substring(0,list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").length()-1));
