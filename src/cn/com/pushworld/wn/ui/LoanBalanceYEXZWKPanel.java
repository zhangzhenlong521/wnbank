 ...
 * @author zzl
 * 贷款余额新增完成比
 * 2019-5-24-上午11:38:04
 */
public class LoanBalanceYEXZWKPanel extends AbstractWorkPanel implements ActionListener{
		...
		if(actionevent.getSource()==list.getQuickQueryPanel()){
			...
			try {
				...
				if(data.length<=0){
					...
					String str=service.getDKBalanceXZ(list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").substring(0,list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").length()-1));
