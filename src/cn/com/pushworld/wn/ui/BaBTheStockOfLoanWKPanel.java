 ...
 * @author zzl
 *
 * 2019-5-27-下午03:30:34
 * 收回存量不良贷款完成比
 */
public class BaBTheStockOfLoanWKPanel extends AbstractWorkPanel implements ActionListener{
		...
		if(actionevent.getSource()==list.getQuickQueryPanel()){
			...
			try {
				...
				if(data.length<=0){
					...
					String str=service.getTheStockOfLoan(list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").substring(0,list.getQuickQueryPanel().getCompentRealValue("DATE_TIME").length()-1));
