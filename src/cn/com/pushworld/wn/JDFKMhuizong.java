...
public class JDFKMhuizong extends AbstractWorkPanel implements ActionListener {
	...
	public void initialize() {
		...
		listPanel.addBatchBillListButton(new WLTButton[] { updateButton, removeButton, submitButton });
	...
	private void QuickQuery() {
		...
		String GY_Name=ClientEnvironment.getInstance().getLoginUserCode();//获取到当前用户的code
	...
	private void updateData() {
		try {
			...
			if (state.equals("未提交") || state.equals("已退回")) {
				BillCardPanel cardpanel = new BillCardPanel("WN_JDFKMHZB_02_ZPY");
