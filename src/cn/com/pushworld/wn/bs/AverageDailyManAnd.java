...
public class AverageDailyManAnd {
	...
	public HashMap<String, String> getComputeMap(String date){
		...
		try {
			...
			//防止重复计算，故做删除
			HashVO [] tjvos=dmo.getHashVoArrayByDS(null,"select * from WN_RJ_CKYXHSTJ where E='"+time[1].toString()+"'");
			if(tjvos.length>0){
				dmo.executeUpdateByDS(null,"delete from WN_RJ_CKYXHSTJ where E='"+time[1].toString()+"'");
			}
			HashVO [] ckbi=dmo.getHashVoArrayByDS(null,"select * from WN_deposit_balance where date_time='"+time[1].toString()+"'");
			if(ckbi.length>0){
			...
			for(int i=0;i<vo.length;i++){
				...
				if(jfmap.size()>0){
					...
					if(jfmap.get(vo[i].getStringValue("name").trim())!=null){
						count=Double.parseDouble(jfmap.get(vo[i].getStringValue("name").trim()));//已计发的Map
					}
					if(Wjfmap.get(vo[i].getStringValue("name").trim())!=null){
						wcount=Double.parseDouble(Wjfmap.get(vo[i].getStringValue("name").trim()));//未计发的Map
					}
					if(((a-b)-count+wcount)>300){
						insert.putFieldValue("C","300");//已计发的
						insert.putFieldValue("D",((a-b)-count+wcount)-300);//没有计发的
						map.put(vo[i].getStringValue("name").trim(), String.valueOf("300"));
						...
						map.put(vo[i].getStringValue("name").trim(), String.valueOf(((a-b)-count+wcount)));
					}
				}else{
					if((a-b)>300){
						insert.putFieldValue("C","300");//已计发的
						...
						map.put(vo[i].getStringValue("name").trim(), String.valueOf((a-b)));
					}
				}
		...
		try {
			for(int i=0;i<vo.length;i++){
				...
				if(DSHQDSMap.get(vo[i].getStringValue("name").trim())!=null){
					a=Double.parseDouble(DSHQDSMap.get(vo[i].getStringValue("name").trim()).toString());
				}
				if(DSHQFQMap.get(vo[i].getStringValue("name").trim())!=null){
					a=a+Double.parseDouble(DSHQFQMap.get(vo[i].getStringValue("name").trim()).toString());
				}
				if(DSDQDSMap.get(vo[i].getStringValue("name").trim())!=null){
					a=a+Double.parseDouble(DSDQDSMap.get(vo[i].getStringValue("name").trim()));
				}
				if(DSDQFQMap.get(vo[i].getStringValue("name").trim())!=null){
					a=a+Double.parseDouble(DSDQFQMap.get(vo[i].getStringValue("name").trim()));
				}
				if(DGHQMap.get(vo[i].getStringValue("code").trim())!=null){
					a=a+Double.parseDouble(DGHQMap.get(vo[i].getStringValue("code").trim()));
				}
				if(DGDQMap.get(vo[i].getStringValue("code").trim())!=null){
					a=a+Double.parseDouble(DGDQMap.get(vo[i].getStringValue("code").trim()));
				}
				map.put(vo[i].getStringValue("name").trim(), String.valueOf(a));
			}
			for(String str:map.keySet()){
				System.out.println(">>>>>>>"+str+">>总计>>>>>>"+map.get(str));
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
