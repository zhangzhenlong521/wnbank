 ...
 */
	...
	public HashMap<String, String> getDGDQRJ(String date) {
		...
		try { //本月天数                                                                                                                                                                                                                                                                                                             //考核月月初日期             
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no oact_inst_no, (sum(acct_bal)/" + time[6] + ") as acct_bal  from wnbank.a_agr_dep_acct_ent_fx where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[0] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[1]
	...
	public HashMap<String, String> getDSDQRJ(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, (sum(acct_bal)/" + time[6] + ") as acct_bal  from wnbank.a_agr_dep_acct_psn_fx where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[0] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[1]+ "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
	...
	public HashMap<String, String> getDSHQRJ(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, (sum(f)/" + time[6] + ") as acct_bal  from wnbank.a_agr_dep_acct_psn_sv where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[0] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[1]+ "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
