 ...
 */
	...
	/**
	 * 对公存款定期部门日均余额-考核月实点Map getDGDQRJKHY
	...
	/**
	 * 对公存款活期部门日均余额-考核月实点Map getDGHQRJKHY
	 * 月初日期 月末日期  上月的月初日期 上月的月末日期  年初月初日期  年末日期  当月天数 上月天数 年初天数
	 */
	public HashMap<String, String> getDGHQRJKHY(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, sum(acct_bal) as acct_bal  from wnbank.a_agr_dep_acct_ent_sv where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')='" + time[1] + "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
	...
	/**
	 * 对私存款定期部门日均余额-考核月实点Map getDSDQRJKHY
	 * 月初日期 月末日期  上月的月初日期 上月的月末日期  年初月初日期  年末日期  当月天数 上月天数 年初天数
	 */
	public HashMap<String, String> getDSDQRJKHY(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, sum(acct_bal) as acct_bal  from wnbank.a_agr_dep_acct_psn_fx where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')='" + time[1] + "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
	...
	/**
	 * 对私存款活期部门日均余额-考核月实点Map getDSHQRJKHY
	 * 月初日期 月末日期  上月的月初日期 上月的月末日期  年初月初日期  年末日期  当月天数 上月天数 年初天数
	 */
	public HashMap<String, String> getDSHQRJKHY(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, sum(f) as acct_bal  from wnbank.a_agr_dep_acct_psn_sv where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')='" + time[1] + "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
