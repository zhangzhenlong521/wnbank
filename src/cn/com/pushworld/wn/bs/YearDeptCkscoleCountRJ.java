 ...
 */
	...
	/**
	 * 对公存款活期部门日均余额-年初Map          getDGHQRJNC 
	 * 年初月初日期:12月1号 年末日期:12月31号
	...
	public HashMap<String, String> getDGDQRJNC(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no oact_inst_no, (sum(acct_bal)/31) as acct_bal  from wnbank.a_agr_dep_acct_ent_fx where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[4] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[5]+ "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
	...
	/**
	 * 对私存款定期部门日均余额-年初Map  getDSDQRJNC
	 * 月初日期 月末日期  上月的月初日期 上月的月末日期  年初月初日期  年末日期  当月天数 上月天数 年初天数
	 */
	public HashMap<String, String> getDSDQRJNC(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, (sum(acct_bal)/31) as acct_bal  from wnbank.a_agr_dep_acct_psn_fx where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[4] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[5]+ "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
	...
	/**
	 * 对私存款活期部门日均余额-年初Map getDSHQRJNC
	 * 月初日期 月末日期  上月的月初日期 上月的月末日期  年初月初日期  年末日期  当月天数 上月天数 年初天数
	 */
	public HashMap<String, String> getDSHQRJNC(String date) {
		...
		String[] time = date.split(";");
		try {
			map = dmo.getHashMapBySQLByDS(null, "select c.code code,a.acct_bal acct_bal from(select oact_inst_no, (sum(f)/31) as acct_bal  from wnbank.a_agr_dep_acct_psn_sv where to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')>='" + time[4] + "' and to_char(to_date(load_dates,'yyyy-mm-dd'),'yyyy-mm-dd')<='" + time[5]+ "' group by oact_inst_no) a left join excel_tab_28 b on a.oact_inst_no=b.a left join pub_corp_dept c on substr(b.b,12)=c.name");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return map;
